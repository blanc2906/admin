...
import { RuleInstance, configurations } from '../configurations/_index';
...
export class MConfigService implements OnModuleInit {
  ...
  async get<T>(key: string): Promise<T> {
    ...
    if (!value) {
      const variable = await this.prismaService.configuration.findUnique({
        where: { key },
      });
      if (variable) {
        ...
        throw new Error(
          `Configuration variable with key ${key} not found in database`,
        );
      }
    }
    return value as T;
